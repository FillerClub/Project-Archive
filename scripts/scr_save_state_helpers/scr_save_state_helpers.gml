...
function update_instance_from_data(instanceId, savedData, saveVars, ignoreVars, saveStateTime = -1) {
	var currentTime = 0;
	with obj_battle_handler	{
		currentTime = get_timer() -game_clock_start;
	}
	with instanceId {
        for (var i = 0, len = array_length(saveVars); i < len; i++) {
            ...
            if struct_exists(savedData, varName) {
				...
				//show_debug_message("Current Time: " +string(currentTime/1000000) +"\nPast Time: " +string(saveStateTime/1000000) +"\n" +string(is_int64(valueSet)));
				if is_numeric(valueSet) && saveStateTime > 0 && currentTime > 0 {
        ...
        if struct_exists(savedData, varName) {
			...
			if is_numeric(valueSet) && saveStateTime > 0 && currentTime > 0 {
				...
				if isTimeValue {
					...
					//show_debug_message("Time Variable Set: " +string(valueSet));
				}
